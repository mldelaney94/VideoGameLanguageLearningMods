...
namespace ShadowrunDragonfallLanguageEngage
{
  ...
  internal static class UICameraProcessMousePatch
  {
    ...
    private static void Postfix(MouseOrTouch[] ___mMouse, RaycastHit ___lastHit)
    {
      ...
      {
        lastWord = word;
        ShadowrunDragonfallLanguageEngage.Log.LogInfo($"{lastWord}");
    ...
    private static int PointIsInBoxes(Vector3 localPoint, BetterList<Vector3> boxes)
    {
      if (boxes.size % 4 != 0)
      {
        ShadowrunDragonfallLanguageEngage.Log.LogWarning($"Box collection size modulo 4 should be 0. Is ({boxes.size} % 4 == {boxes.size % 4})");
      }
